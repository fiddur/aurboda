 ...
 * Also applies auto-tagging rules to create activities from scrobbles.
 */
...
const applyPointInTimeRules = async (
  ...
  const createdActivities = new Set<string>()

  for (const scrobble of scrobbles) {
    for (const rule of rules) {
      if (matchesRule(scrobble, rule)) {
        const activityKey = `${rule.tag_name}|${scrobble.timestamp.toISOString()}`
        if (createdActivities.has(activityKey)) continue
        createdActivities.add(activityKey)

        const activityType = await resolveOrCreateActivityType(user, rule.tag_name)
 ...
 * Groups matching scrobbles into sessions and creates span activities.
 *
...
const applySessionRules = async (
  ...
  for (const rule of rules) {
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      if (i === 0) {
        const existingActivity = await findMergeableActivity(
          user,
          activityType,
          session.startTime,
        ...
        if (existingActivity?.external_id) {
          await updateActivityEndTimeByExternalId(user, existingActivity.external_id, sessionEnd)
          activitiesCreated++
          continue
        }
      }
...
/**
 * Apply tag rules to scrobbles and create activities.
...
/**
 * Hard-delete all activities generated by a specific rule.
 * Called before a rule is deleted.
 */
export const cleanupRuleActivities = async (user: string, ruleId: string): Promise<number> => {
  const pointDeleted = await hardDeleteActivitiesByExternalIdPrefix(
    user,
    'lastfm-auto',
    `lastfm-auto-${ruleId}-`,
  )
  const sessionDeleted = await hardDeleteActivitiesByExternalIdPrefix(
    user,
    'lastfm-auto',
    `lastfm-session-${ruleId}-`,
  )
  return pointDeleted + sessionDeleted
}

/**
 * Delete all auto-generated Last.fm activities and reapply all rules from scratch.
 * Use when rules have changed and activities need a full refresh.
 */
export const retagAllScrobbles = async (user: string): Promise<LastFmRetagResult> => {
  const tagsDeleted = await hardDeleteActivitiesBySource(user, 'lastfm-auto')

...
- **View** a shared dashboard at `/u/<username>/<slug>` and a profile at
  ...
  app chrome (header/sidebar/footer) — a clean, standalone page — and fetch
  nothing per widget (they render from the server-resolved data). A logged-in
  viewer keeps their normal app nav on these pages, so they can navigate away
